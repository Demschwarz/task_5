...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
      ...
      if (!empty($_COOKIE['pass'])) {
          $messages[] = sprintf('You can be authorised с логином <strong>%s</strong>
        and password <strong>%s</strong> for data change.',
  ...
  if ($errors['fio']) {
    ...
    $messages[] = '<div class="mesa">Fill the name.</div>';
    else $messages[] = '<div class="mesa">Name in russian</div>';
  }
  
  if ($errors['email']) {
      setcookie('email_error', '', 100000);
      if($_COOKIE['email_error'] == 1)
          $messages[] = '<div class="mesa">Fill Email.</div>';
          else $messages[] = '<div class="mesa">Wrong Email</div>';
  }
  
  if ($errors['bday']) {
      setcookie('bday_error', '', 100000);
      if($_COOKIE['bday_error'] == 1)
          $messages[] = '<div class="mesa">Fill BirthDate.</div>';
          else $messages[] = '<div class="mesa">Wrogn BirthDate</div>';
  }
  
  if ($errors['sex']) {
      setcookie('sex_error', '', 100000);
      if($_COOKIE['sex_error'] == 1)
          $messages[] = '<div class="mesa">Choose gender.</div>';
          else $messages[] = '<div class="mesa">Wrong gender </div>';
  }
  ...
  if ($errors['lim']) {
      ...
      if($_COOKIE['lim_error'] == 1)
          $messages[] = '<div class="mesa">Choose lungs quantity.</div>';
          else $messages[] = '<div class="mesa">Wrong lungs quantity.</div>';
  }

  if ($errors['abil']) {
      setcookie('abil_error', '', 100000);
      if($_COOKIE['abil_error'] == 1)
          $messages[] = '<div class="mesa">Choose Superskill.</div>';
          else $messages[] = '<div class="mesa">Wrong superskill format.</div>';
  }
  
  if ($errors['bio']) {
      setcookie('bio_error', '', 100000);
      $messages[] = '<div class="mesa">Fill the biography.</div>';
  }
  
  if ($errors['yes']) {
      setcookie('yes_error', '', 100000);
      $messages[] = '<div class="mesa">Choose "I agree"</div>';
  }
  ...
  $ability_labels = ['god' => 'Immortality', 'twalk' => 'Wallhack', 'fly' => 'Levitation' ];
  ...
  if (!empty($_COOKIE[session_name()]) &&
      session_start() && !empty($_SESSION['login'])) {
          ...
          $userb = 'u20419';
          $passb = '9620609';
          $db = new PDO('mysql:host=localhost;dbname=u20419', $userb, $passb, array(PDO::ATTR_PERSISTENT => true));
